...
{
    /**
     * Act - do whatever the dddd wants to do. This method is called whenever
    ...
    public void read(HashMap<String, Key> Sites){
        ...
        try {
            BufferedReader in = new BufferedReader(new FileReader("musiclist/"+ MusicName + ".txt"));
            String str;
            while ((str = in.readLine()) != null) {
                ...
                if(key!=null){
                    ...
                    thirdTimer.schedule(new TimerTask(){
                        public void run() {
                            Greenfoot.playSound(""+name.charAt(name.length() - 1) + "/" +name+".mp3");
                        }
                    },  time);
