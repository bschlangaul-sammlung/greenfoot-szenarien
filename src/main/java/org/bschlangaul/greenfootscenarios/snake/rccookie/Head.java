...
public class Head extends Actor
{
    ...
    public void act()
    {
        if (alive)
        {
            ...
            {
                /*
                 * if(getOneObjectAtOffset((int)Math.cos(direction*90), (int)Math.sin(direction*90),
                 * Body.class)!=null) {
                 *
                 * } else { turnTowards(getOneObject(Food.class).getX(),
                 * getOneObject(Food.class).getY()); }
                 */
            }
            else
            {
    ...
    private void foundFood()
    {
        ...
        getWorld().addObject(new Food(), r.nextInt(MyWorld.getMyWidth()),
                        r.nextInt(MyWorld.getMyHeight()));
    }

    public boolean isAtEdge()
    {
        if (getX() < 0 || getX() >= MyWorld.getMyWidth() || getY() < 0
                        || getY() >= MyWorld.getMyHeight())
            return true;
        return false;
    }
