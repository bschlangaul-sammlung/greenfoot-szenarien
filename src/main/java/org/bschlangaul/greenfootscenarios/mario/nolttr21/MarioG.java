...
/**
 * MarioG is the ultimate mario character, equipped with a gun. He is the main actor. If he is hit, he will turn into regular mario
 * 
...
public class MarioG extends AllMarios
{
    ...
    public boolean marioSetAsGun = false; //this boolean shows whether or not sWorld has had its variables set for mario having a gun
    ...
    /**
     * check if we are touching the goal posts
     * if we are, play the winning sound and set the world back to the map
     */
    ...
    private void animateMario()
    {
        ...
        if (Greenfoot.isKeyDown("left"))
        {
            if (imageDelayCount >= imageTime)//if it's time to change the image
            {
                ...
                setImage(characterName + "mariogunL" + imageNumber + ".png");//it looks like we're walking
    ...
    /**
     * This will take ammo away from mario's gun and update the scoreboard based on which world mario is in
     */
    ...
    /**
     * This will shoot a bullet from mario's gun if the delay count is greater than or equal to the reload time and if the gun has bullets to shoot
     */
    public void shoot()
    {
        SWorld sWorld = (SWorld)getWorld();
