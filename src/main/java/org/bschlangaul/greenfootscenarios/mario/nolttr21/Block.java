...
public class Block extends Solid
{
    ...
    /**
     * This will check if the block has been hit by mario If the block was hit by mario or marioG,
     * then the block will create ammo or a heart, based on a random number. If the block was hit by
     * marioS, then the block will create a mushroom or a heart, based on a random number.
     */
    public void checkHit()
    {
        if (getOneObjectAtOffset(0, getImage().getHeight() / 2 + 1, MarioG.class) != null
                        && getImage() != imageSpent)
        {
            bumped = true;
        }
        ...
        if (getOneObjectAtOffset(0, getImage().getHeight() / 2 + 1, MarioS.class) != null
                        && getImage() != imageSpent)
        {
            bumped = true;
        }
