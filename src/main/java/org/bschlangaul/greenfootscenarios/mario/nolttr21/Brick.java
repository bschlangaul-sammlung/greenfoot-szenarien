...
public class Brick extends Solid
{
    ...
    public void act() 
    {
        if (getWorld() instanceof Fortress && changed == false) //changed variable added to reduce lag
        {
            setImage(bricks);
            changed = true;
        }
        
        if (getWorld() instanceof KoopaWorld && changed == false) //changed variable added to reduce lag
        {
            setImage(bricks);
            changed = true;
        }
        
        if (getWorld() instanceof BossWorld2 && changed == false) //changed variable added to reduce lag
        {
            setImage(bricks);
            changed = true;
        }
        
        if (getWorld() instanceof BossWorld && changed == false) //changed variable added to reduce lag
        {
            setImage(bricks);
            changed = true;
        }
