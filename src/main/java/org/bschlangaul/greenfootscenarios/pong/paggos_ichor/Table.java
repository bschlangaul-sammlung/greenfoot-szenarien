...
public class Table extends World
{
    ...
    public void act()
    {
        // player.setLocation(player.getX(),Greenfoot.getMouseInfo().getY());
        ...
        if (ball.getY() >= computer.getY() - 25 && ball.getY() <= computer.getY() + 25
                        && ball.getX() + dx >= computer.getX())
        {
            dx = -dx;
            dy = (ball.getY() - computer.getY()) / 2;
            Greenfoot.playSound("beep.wav");
        }
        if (ball.getY() >= player.getY() - 25 && ball.getY() <= player.getY() + 25
                        && ball.getX() + dx <= player.getX())
        {
            dx = -dx;
