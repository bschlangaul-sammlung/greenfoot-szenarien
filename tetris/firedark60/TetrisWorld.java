...
public class TetrisWorld extends World
{
     ...
     * This creates more of the permanent objects in the world, and starts up the Actors.
     * 
     */
    public TetrisWorld()
    {    
        ...
        if (hasScoreBoardAppeared == false ) {
            ...
            //Holds a shape so player can use later, Can only hold once per turn
                ...
                if (holdShapeNumber == 0) {
                    ...
                    holdShape.getImage().scale (holdShape.getImage().getHeight()/2,holdShape.getImage().getWidth()/2);
    ...
    /**
     * Method delays
     * Delay time of drop shapes, with the delay depending on the level difficulty
     *
     */
    public void delays () {
    ...
    /**
     * Method finishWithShape
     *This registers the shape, and if the shape is in front of the spawning area, then it goes to Game Over
     */
    public void finishWithShape () {
        ...
        //If a piece is in the spawning area, then Game Over
            ...
            addObject(new ScoreBoard("Game Over",score.getValue(), hiScore.getValue ()), getWidth()/2, getHeight()/2);
            hasScoreBoardAppeared = true;
        }
    ...
    /**
     * Method addShape
     * Adds a new Current Shape to the game
    ...
    public void beginNextShapes () {
        ...
        //Scale to fit
        for (int i = 0; i < 6; i++ ) {
            nextShape[i].getImage().scale (nextShape[i].getImage().getHeight()/2,nextShape[i].getImage().getWidth()/2);
        }
    ...
    /**
     * Method nextShapes
     * Makes the next Shape into the current Shape, and moves all the next Shape up by 1
     */
    public void nextShapes () { 
        ...
        //Scale to fit
        nextShape[5].getImage().scale (nextShape[5].getImage().getHeight()/2,nextShape[5].getImage().getWidth()/2);
        addObject(nextShape[5], 435, 210);
    ...
    /**
     * Method lineCheck
     * Checks for and clear filled Line, adds up the points, changes the level , and change the level music if needed.
     */
    public void lineCheck () {
        ...
        //Depending how many lines cleared and how many time lines were cleared per turn, points are scored
        if (linesCleared == 0) { 
            combo.setValue (0);
        }
        ...
        if (lines.getValue () <= 0) {
            ...
            if (level.getValue () ==  11) {
                if (hasScoreBoardAppeared == false) {
                    ...
                    addObject(new ScoreBoard("You Win!",score.getValue(), hiScore.getValue ()), getWidth()/2, getHeight()/2);
                    hasScoreBoardAppeared = true;
                }
    ...
    /**
     * Method createBoarder
     * Intializes the Boarder of the game, so the blocks don't fall out of the game
     */
